...
public class PullUpToLoadMore extends ViewGroup {
	...
	ScrollPageTurnListener mScrollPageTurnListener;

	public void setScrollPageTurnListener(
			ScrollPageTurnListener mScrollPageListener) {
		this.mScrollPageTurnListener = mScrollPageListener;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getScrollY() + dy + getHeight() > bottomScrollView.getBottom()) {
				dy = dy
						- (getScrollY() + dy - (bottomScrollView.getBottom() - getHeight()));
			}
			scrollBy(0, dy);
			break;
		case MotionEvent.ACTION_UP:
			...
			if (currPosition == 0) {
				if (yVelocity < 0 && yVelocity < -speed) {
					...
					currPosition = 1;
					if (null != mScrollPageTurnListener) {
						mScrollPageTurnListener
								.onScrollPageTurnListener(ScrollPageTurnListener.TurnDown);
					}
				} else {
					smoothScroll(0);
				}
				...
				if (yVelocity > 0 && yVelocity > speed) {
					...
					if (null != mScrollPageTurnListener) {
						mScrollPageTurnListener
								.onScrollPageTurnListener(ScrollPageTurnListener.TurnUp);
					}
	...
	public interface ScrollPageTurnListener {
		public static final int TurnUp = 0;
		public static final int TurnDown = 1;
