...
import com.delevin.shenghuidai.adapter.ChongzhiJiluAdapter;
...
public class ChongzhiJiluActivity extends BaseActivity {
	...
	private List<BeanChongzhiJilu> datas;
	...
	protected void findViews() {
		...
		datas = new ArrayList<BeanChongzhiJilu>();
		zijinListView.setSelector(new ColorDrawable(Color.TRANSPARENT));
		zijinListView.setDividerHeight(0);
		adapter = new ChongzhiJiluAdapter(getApplicationContext(), datas, R.layout.item_chongzhi_jilu);
			...
			public void onResponse(JSONObject jsonObject) {
				try {
					...
					if (jsonObject.getString("code").equals("10000")) {
						...
						List<BeanChongzhiJilu> list = null;
						if (array.length() == 0) {
							if (index == 1) {
							...
							list = JSON.parseArray(array.toString(), BeanChongzhiJilu.class);
							datas.addAll(list);
