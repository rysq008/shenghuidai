...
import com.calendar.interf.OnAdapterSelectListener;
...
public class CalendarViewAdapter extends PagerAdapter {
	...
	public Object instantiateItem(ViewGroup container, int position) {
		...
		if (calendarType == CalendarAttr.CalendayType.MONTH) {
			CalendarDate current = seedDate.modifyMonth(position
					- MonthPager.CURRENT_DAY_INDEX);
			current.setDay(1);// 每月的种子日期都是1号
			calendar.showDate(current);
		} else {
			CalendarDate current = seedDate.modifyWeek(position
					- MonthPager.CURRENT_DAY_INDEX);
			if (weekArrayType == 1) {
	...
	public void setSignInData(HashMap<String, String> signinData) {
		Utils.setSignInData(signinData);
	}
